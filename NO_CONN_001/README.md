...
| `Deliverables/` | The indicator in the ecosystemCondition platform format - region map (`.rds`, EPSG:25833), values table (csv/xlsx with metadata), figure; the polygon-level map (1.15 M rows, 51 MB) is gitignored and regenerated by `Main/export_deliverables.R`. |
...
"Deliverables" below) - a few minutes, most of it bootstrapping.
...
- `v` = kvotient (distance to nearest infrastructure / distance to
  ...
  *geometric* means over polygons with a finite, non-zero ratio; the
  ...
  `i` but have no `v`.
- `reference_high` / `reference_low` = 177.7 / 0.141 on the variable
  scale: the 99th / 1st percentile of log kvotient from the national run
  itself. The scaling (linear on the log, no sigmoid) is this
  reconstruction's - the original never finalised one (issue #144).
...
- **Nord-Norge is not comparable to the other regions**: the national
  mire probability map stops at ~64 N, so its polygons are NiN-mapped
  mires only (4,917 vs 200-450 k elsewhere; 48.6 % within 1000 m of
  infrastructure vs ~10 %). Its value describes that subset. Stated in the
  metadata sheet as "Coverage caveat".
