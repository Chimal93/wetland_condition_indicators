...
# ===========================================================
# WRITE-OUT (no computation happens here). Data/connectivity_scored/
# (gitignored - 450 MB, regenerated by this script in minutes):
#   mire_scored_<label>.gpkg      every mire polygon with kvotient, index, index_note
#   region_agg_<label>.csv        region area-weighted mean index, n, bootstrap CI
#   anchors_<label>.csv           the percentile anchors this run used
# <label> = CONNECTIVITY_SCORE_LABEL env var (default "national_2023").
# Main/export_deliverables.R reads these.
# ===========================================================
score_label <- Sys.getenv("CONNECTIVITY_SCORE_LABEL", unset = "national_2023")
scored_dir  <- file.path("..", "Data", "connectivity_scored")
if (!dir.exists(scored_dir)) dir.create(scored_dir, recursive = TRUE)

st_write(mire_scored, file.path(scored_dir, paste0("mire_scored_", score_label, ".gpkg")),
         delete_dsn = TRUE, quiet = TRUE)
write.csv(as.data.frame(region_agg), file.path(scored_dir, paste0("region_agg_", score_label, ".csv")), row.names = FALSE)
anch <- attr(mire_scored, "anchors")
write.csv(data.frame(X0_log = anch[["X0_log"]], X100_log = anch[["X100_log"]],
                     X0_kvotient = exp(anch[["X0_log"]]), X100_kvotient = exp(anch[["X100_log"]]),
                     percentile_lower = PERCENTILE_LOWER, percentile_upper = PERCENTILE_UPPER,
                     n_scored = n_scored, is_final = is_final),
          file.path(scored_dir, paste0("anchors_", score_label, ".csv")), row.names = FALSE)
cat("\nWrite-out:", nrow(mire_scored), "scored polygons ->", scored_dir, "(label", score_label, ")\n")
